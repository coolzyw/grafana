...
void create_main_page_table(MYSQL *conn);

void insert_value_into_main_page_table(MYSQL *conn);
...
int main(int argc, char* argv[]){
        ...
        if(conn){
                ...
                while(1){
                        ...
                        insert_value_into_main_page_table(conn);
                        sleep(5);
                }
...
{
    char drop_table[100];
    strcpy(drop_table, "DROP TABLE IF EXISTS main_page");
    mysql_query(conn, drop_table);
    char create_table[100];
    strcpy(create_table, "CREATE TABLE main_page (link VARCHAR(50) NOT NULL, machine_number INT, 
        latency INT, throughput INT)");
    mysql_query(conn, create_table);
}

void insert_value_into_main_page_table(MYSQL *conn){
    int latency = rand()%100;
    int throughput = rand()%5;
}